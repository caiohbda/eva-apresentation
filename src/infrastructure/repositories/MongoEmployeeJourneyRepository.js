...
const EmployeeJourneySchema = new mongoose.Schema({
  ...
  journeyId: { type: mongoose.Schema.Types.ObjectId, required: true, ref: 'Journey' },
...
// Modelo do MongoDB
const EmployeeJourneyModel = mongoose.model('EmployeeJourney', EmployeeJourneySchema);
...
const MongoEmployeeJourneyRepository = () => {
  const save = async (employeeJourney) => {
    ...
    const savedEmployeeJourney = await EmployeeJourneyModel.create(employeeJourneyData);
    return EmployeeJourney.withId(employeeJourney, savedEmployeeJourney._id.toString());
  };
  ...
  const findById = async (id) => {
    ...
    return EmployeeJourney.withId({
      ...
      status: employeeJourney.status,
      currentActionIndex: employeeJourney.currentActionIndex,
      completedActions: employeeJourney.completedActions.map(id => id.toString())
  ...
  const update = async (id, employeeJourney) => {
    ...
    return EmployeeJourney.withId(employeeJourney, updatedEmployeeJourney._id.toString());
  };
